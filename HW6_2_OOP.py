...
class Pipe():
    ...
    def Re(self):
        ...
        '''
        self.reynolds = (
                                    self.V() * self.length) / 0.00000089  # $JES MISSING CODE$ # Re=rho*V*d/mu, be sure to use V() so velocity is updated.
    ...
    def frictionHeadLoss(self):  # calculate headloss through a section of pipe in m of fluid
        ...
        hl = ff * (self.length / self.d) * (
                    self.vel ** 2 / (2 * g))  # $JES MISSING CODE$ # calculate the head loss in m of water
        return hl
